...
class Payment:
    def __init__(self, date, mode, checkNumber, amount):
        self.date = date
        self.mode = mode
...
def AddPaymentToStudentRecord(paymentNo,PaymentData, studentrecord):
    ...
    payment = Payment(paymentNo, PaymentData.PaymentDate.date(), PaymentData.PaymentForm, PaymentData.CheckNumber, PaymentData.Amount  )
    ...
    return studentrecord.append
'''
def AddStudentRecordToStudents_df(studentRecord):
    expanded_record = {"StudentID": studentRecord["student"], "Payment mode1": studentRecord["Payments"][1],"Payment1 date", "Amount1", "Cheque1 No": }
    sourceDf["Payment mode1"]
...
def Student_payments(PaymentsExcelFile):
    ...
    'gp = df.groupby("StudentID").apply(lambda x: x.sort_values(["PaymentDate"]))'
    'print(gp.head(5))'
    'print(gp.iloc[2])'
    StudentsPayments_df =pd.DataFrame()
    for student, payments in gp:
        'student_record = {"StudentID": student, "Payments": [payment1,payment2,.....]}'
        p = [None] * 12
        student_record = {"StudentID": student, "Payments" : p}
        c  = 0
        for row, data in payments.iterrows():
            c = c + 1
            PaymentData = df.loc[row]
            payment = Payment(PaymentData.PaymentDate.date(), PaymentData.PaymentForm, PaymentData.CheckNumber, PaymentData.Amount)'
            student_record["Payments"][c] = payment
        StudentsPayments_df = StudentsPayments_df.append(student_record, ignore_index = True)
    'WriteExcelRecords("StudentPayments.xlsx",StudentPayments_df)'
    return StudentsPayments_df
...
def main():
    ...
    Temp_df = TemplateDataFrame(full_df)
