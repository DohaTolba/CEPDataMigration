...
def AddPaymentToStudentRecord(paymentNo ,PaymentData, studentrecord):
    ...
    if PaymentData["PaymentType"] in [33,41,55] :
        studentrecord["Discount"] = studentrecord["Discount"] + PaymentData["Amount"]
...
def complet12(c,student_record):
    c = c+1
    PaymentData = {"PaymentType": None,"PaymentDate": None, "Amount": None, "CheckNumber": None, "PaymentForm": None}
    for i in range(c,13): 
...
def TemplateDataFrame(sourceDf):
    ...
    destinationDf["Discount type"] = sourceDf["Discount"]
